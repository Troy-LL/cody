...
class CursorOverlay(QWidget):
    ...
    def _paint_cursor(self, p: QPainter) -> None:
        ...
        if self._pointing:
            ...
            path.lineTo(32, 26)
            path.closeSubpath()
            face_c = QPointF(34, 14)
        p.setPen(QPen(QColor("#0A1628"), 1.5))
        p.setBrush(QColor("#1B4FD8"))
        p.drawPath(path)
        p.setBrush(QColor("#E8F0FE"))
        p.setPen(QPen(QColor("#1B4FD8"), 2))
        p.drawEllipse(face_c, 9, 9)
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(QColor("#0A1628"))
        p.drawEllipse(face_c + QPointF(-2.5, -1), 1.4, 1.4)
        p.drawEllipse(face_c + QPointF(2.5, -1), 1.4, 1.4)
        p.setPen(QPen(QColor("#0A1628"), 1.4))
        smile = QPainterPath()
        smile.moveTo(face_c.x() - 3, face_c.y() + 2.5)
        smile.quadTo(face_c.x(), face_c.y() + 5.5, face_c.x() + 3, face_c.y() + 2.5)
        p.drawPath(smile)
        if self._pointing:
            p.setPen(Qt.PenStyle.NoPen)
